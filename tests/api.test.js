...
describe('Social Media Downloader API', () => {
  ...
  describe('User Endpoints', () => {
    ...
    it('should succeed /tiktok with valid token and URL', async () => {
      const response = await request(app)
        .post('/tiktok')
        .set('x-api-token', userToken)
        .send({ url: 'https://www.tiktok.com/@food9184/video/7574637317005036830?is_from_webapp=1' });
      expect(response.status).toBe(200);
    ...
    it('should fail with expired token', async () => {
      ...
      const expiredToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6ImV4cGlyZWRAZXhhbXBsZS5jb20iLCJpYXQiOjE3MTAwMDAwMDAsImV4cCI6MTcxMDAwMDAwMX0.invalid-signature';
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle database errors in /admin/generate-token', async () => {
      ...
      const response = await request(app)
        .post('/admin/generate-token')
        .send({ email: 'db-error@test.com', allowedRequests: 10, adminEmail, adminPass });
      expect(response.status).toBe(200);
    });
